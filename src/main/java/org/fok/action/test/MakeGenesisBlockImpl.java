...
@Slf4j
@Data
public class MakeGenesisBlockImpl extends SessionModules<ReqMakeNewBlock> {
	...
	public String[] getCmds() {
		return new String[] { ActionTestCommand.NGB.name() };
	}
	...
	public void onPBPacket(final FramePacket pack, final ReqMakeNewBlock pb, final CompleteHandler handler) {
		...
		try {
			if (fokBlockChain.getBlockByHeight(0) == null) {
				BlockInfo bi = fokBlock.createGenesisBlock();
				...
				oRespMakeNewBlock.setRetMsg("创世块");
			} else {
				oRespMakeNewBlock.setRetMsg("已存在创世块，不允许重复创建");
				oRespMakeNewBlock.setRetCode(-1);
			}
		} catch (Exception e) {
			log.error("", e);
