...
import org.fok.action.account.GetAccountByAddressImpl;
...
@Slf4j
@Data
public class GetBlockByHashImpl extends SessionModules<BlockHashMessage> {
	...
	public void onPBPacket(final FramePacket pack, final BlockHashMessage pb, final CompleteHandler handler) {
		...
		try {
			...
			BlockInfo bi = fokBlockChain.getBlockByHash(crypto.hexStrToBytes(pb.getHash()));
